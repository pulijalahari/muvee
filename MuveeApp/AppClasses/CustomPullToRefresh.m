...
- (id) initWithScrollView:(UIScrollView *)scrollView delegate:(id<CustomPullToRefreshDelegate>)delegate {
    ...
    if (self) {
        ...
        for (int i=1; i<20; i++)
          [animationImages addObject:[UIImage imageNamed:[NSString stringWithFormat:@"loading-%d",i]]];
   ...
   //     [scrollView addSubview:_rainbowTop];
        ...
        _rainbowBot = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"loading-1"]];
 ...
 //[[NSNotificationCenter defaultCenter] addObserver: self selector: @selector(handleOrientationChangeNotificationTable) name: UIApplicationDidChangeStatusBarOrientationNotification object: nil];
...
//        [_rainbowTop addSubview:_arrowTop];
        ...
        _arrowBot = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"big_arrow"]];
